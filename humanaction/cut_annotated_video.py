...
def cut_both_video_json(video_file, video_dir, annotation_file, keypoint_dir, video_output_dir, json_output_dir, json_only=False):
    ...
    for _, row in anno_video.iterrows():
        ...
        output_name = f"{osp.splitext(video_file)[0]}_segment_{start_frame}_{end_frame}_action_{action}_label_{label}"
        ...
        if json_only:
            ...
            input_video = osp.join(video_dir, video_file)
...
if __name__ == '__main__':
    ...
    video_file = 'VID_20230112_093006.mp4'
    ...
    json_only = True
    cut_both_video_json(video_file, video_dir+'jm', annotation_file, keypoint_dir, video_output_dir, json_output_dir, json_only)
