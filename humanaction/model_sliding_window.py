...
def sliding_window(data, window_size, stride):
    ...
    num_windows = int((nb_frames - window_size) / stride) + 1
    ...
    for epoch in range(nb_epochs):
        ...
        for train in [True, False]:
            ...
            for data in dataloader:
                windows = sliding_window(data[0], 2*24, 24/4)
                
                for window in windows:
                    input = window.to(device)
                    labels = torch.zeros((input.shape[0], len(classes)))
                    ...
                    outputs_for_loss,_,_ = model(input)
...
def main():
    ...
    torch.save(model.state_dict(), "C:\\Users\\Shadow\\Documents\\Projets\\MastereIA\\Idemia\\human-action-recognition\\models_saved\\action_lstm_2D_sliding_window.pt")
    ...
    plt.legend()
    plt.savefig("C:\\Users\\Shadow\\Documents\\Projets\\MastereIA\\Idemia\\human-action-recognition\\models_saved\\action_lstm_2D_sliding_window_loss_acc.png")
