...
video_filename = "vis_video_longue_recadree.mp4"
out_video_filename = "vis_video_longue_recadree_sliding_window_predict.mp4"
...
def main():
    ...
    for data_count, data in enumerate(dataloader):
        ...
        for start, end, window in windows:
            ...
            window_probs = np.concatenate([start*np.ones((1,)), end*np.ones((1,)), probs]).reshape(1,11)
            windows_probs.append(window_probs)
        vid_probs = np.concatenate(windows_probs)
        np.save(os.path.join(output_dir, data2D_files[data_count][:-4]+'.npy'), vid_probs)
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            # predict
            probs = vid_probs_dup[frame_count-1]
